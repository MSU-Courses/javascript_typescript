...
> [!IMPORTANT]
> `undefined` — это примитивный тип данных, который автоматически присваивается переменным при их объявлении или аргументам функции, если значение не было установлено.
...
### Основные особенности
* Переменные, которым не было присвоено значение, по умолчанию имеют значение `undefined`.
* Параметры функции, которым не были переданы значения при вызове, имеют значение `undefined`.
...
```
### Основные особенности
* Позволяет работать с числами, больше, чем $2^{51}-1$ и меньше, чем $-2^{51}-1$.