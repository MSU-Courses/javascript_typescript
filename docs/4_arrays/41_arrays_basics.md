...
**Массивы в JS** — это специальные объекты, которые представляют собой упорядоченные списки элементов, индексированные числовыми значениями (О том, что такое объекты, мы рассмотрим немного позже).
...
Для получения элемента массива в JavaScript также используются квадратные скобки, аналогично другим языкам программирования.
```js
...
Более подробное устройство массива в JS мы поймем, когда изучим объекты. На данный момент стоит, отметить пару важных моментов.

### Хранение в памяти

Массивы в JS хранятся в памяти последовательно, как показано на рисунке во введении, то есть элементы массива располагаются рядом друг с другом в порядке их объявления, что обеспечивает эффективный доступ к элементам по их индексам.
